#the current dataset is not the final dataset that I will be using, this is already a very preped dataset
#but I do know how I will prepare my dataset because the type of data is very similar
#I know what the raw data will be like and the following steps will be the steps I take to prep my data
...
#intermediate_AgeandPossibleSelves_1 <- Age_and_Possible_Selves_9_6_19 %>%
...
#while information on the participants' location, answer duration etc. are also not necessary. filter() out
...
#mutate() , rename() will also be used a lot for a value for the scale
...
#ggplot2()+geom_point()+geom_smooth() for the simple mediation, simple moderation
